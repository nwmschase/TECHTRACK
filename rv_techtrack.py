    ...
    if category_id:
        ...
        if water_heater_context:
            ...
            if extra and extra.id not in cat_ids:
                cat_ids.append(extra.id)
        if cooktop_context:
            for extra_name in ("Range & Cooktops", "Ranges", "Cooktops", "Furnaces", "ID & Reference"):
                extra = session.query(Category).filter(Category.name == extra_name).first()
                if extra and extra.id not in cat_ids:
                    cat_ids.append(extra.id)
        ...
        cat_ids = []
        for extra_name in (
            "Range & Cooktops", "Ranges", "Cooktops", "Furnaces", "ID & Reference", "TSB / Recall"
        ):
            extra = session.query(Category).filter(Category.name == extra_name).first()
            if extra and extra.id not in cat_ids:
                cat_ids.append(extra.id)
    ...
    if asked:
        ...
        if branded:
            ...
            if water_heater_context:
                ...
                extra_names = ["Range & Cooktops", "Ranges", "Cooktops", "Furnaces", "ID & Reference"]
            elif stabilizer_context:
